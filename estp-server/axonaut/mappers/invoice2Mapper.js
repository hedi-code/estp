...
// bc2Data shape:
// {
//   commande : commande2 row  (id, pack2_color, reduc_pct, remise_pack_plus, created)
//   pack     : { nom, coloris, prix_ht }  (pack2s row)
//   options  : [{ nom, prix_ht, taux_tva, qty, color, reduction }]
// }
...
function toAxonautInvoice2(bc2Data, axonautCompanyId) {
  ...
  // Lines 2..N – Selected BC2 options
  // Each option carries its own TVA rate and an optional per-line reduction.
  for (const opt of options) {
    const unitPrice = parseFloat(opt.prix_ht) - parseFloat(opt.reduction || 0);
